...
function Dashboard() {
    ...
    useEffect(() => {
        // eslint-disable-next-line react-hooks/set-state-in-effect
        if (totalPages > 0 && currentPage > totalPages) setCurrentPage(totalPages);}, [totalPages, currentPage]);

    return (
        <div className="space-y-6 sm:space-y-8">
            <header className="flex items-start justify-between gap-3 sm:gap-4">
                <div className="min-w-0">
                    <h1 className="text-xl font-semibold leading-tight tracking-tight text-slate-800 dark:text-white sm:text-2xl">
                        My Tasks
                    </h1>
                ...
                <button
                    type="button"
                    onClick={handlers.handleOpenAddTask}
                    className="hidden shrink-0 items-center gap-2 rounded-lg border border-green-200 bg-green-50 px-3.5 py-2 text-sm font-medium text-green-600 shadow-sm transition hover:border-green-300 hover:bg-green-100 hover:shadow focus:outline-none focus:ring-2 focus:ring-green-200 dark:border-green-800 dark:bg-green-950/50 dark:text-green-400 dark:hover:border-green-700 dark:hover:bg-green-950 xl:inline-flex"
                >
                    <Plus size={16} />
                    Add Task
                </button>
            ...
            <div className="flex min-w-0 items-start gap-2 xl:grid xl:grid-cols-[minmax(0,7fr)_minmax(260px,3fr)] xl:gap-4">
            ...
            <div className="grid min-w-0 gap-8 xl:grid-cols-[minmax(0,7fr)_minmax(260px,3fr)]">
                <section className="min-w-0 space-y-4">
                    {loading ? (
                        <div className="flex min-h-[300px] items-center justify-center rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
            ...
            <DeleteTask
                task={taskToDelete}
                onConfirm={() =>
                    handlers.handleConfirmDelete(taskToDelete)
                }
