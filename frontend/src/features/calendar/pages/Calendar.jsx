...
function Calendar() {
    ...
    return (
        <div className="px-3 py-5 sm:px-6 sm:py-6 lg:px-8 lg:py-8">
            <div className="mb-4 space-y-4 sm:mb-6 sm:flex sm:items-center sm:justify-between sm:space-y-0">
                ...
                <div className="flex items-center justify-between gap-1.5 sm:justify-end sm:gap-2">
                    <button
                        type="button"
                        onClick={goToToday}
                        className="rounded-lg border border-slate-200 px-3 py-2 text-xs font-medium text-slate-600 transition hover:bg-slate-50 sm:px-4 sm:text-sm"
                    >
                        Today
                    </button>
                    ...
                    <h2 className="min-w-0 flex-1 text-center text-sm font-semibold text-slate-800 sm:min-w-40 sm:flex-none sm:text-lg">
                        {monthName} {year}
                    </h2>
            ...
            <div className="grid gap-5 lg:grid-cols-[minmax(0,7fr)_minmax(260px,3fr)]">
                <div className="min-w-0 overflow-hidden rounded-xl border border-slate-200 bg-white sm:rounded-2xl">
                    <div className="grid grid-cols-7 border-b border-slate-200">
                        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map(
                            (day) => (
                                <div
                                    key={day}
                                    className="border-r border-slate-200 px-1 py-2 text-center text-[9px] font-semibold uppercase tracking-wide text-slate-400 sm:px-3 sm:py-3 sm:text-xs"
                                >
                        ...
                        {calendarDays.map((day, index) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`min-h-20 border-b border-r border-slate-200 p-1.5 text-left transition sm:min-h-32 sm:p-3 ${
                                        isToday ? "bg-green-50/50" : ""
                                    } ${
                                        isSelected
                                            ? "ring-2 ring-inset ring-green-300"
                                            : "hover:bg-slate-50"
                                    }`}
                                >
                                    {day && (
